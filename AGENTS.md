...
- Core packages: `@zephyrr-ci/{types,config,core,storage,server,cli,web,vm}`

## Code Style
- **Imports**: Always use `.ts` extension (e.g., `from "./loader.ts"`). Group by: external, workspace (`@zephyrr-ci/*`), relative
