#!/bin/bash
#
# Zephyr CI - Arch Linux Setup Script (Idempotent)
#
# This script installs and configures Zephyr CI on Arch Linux.
...
#
# Usage:
#   Interactive:     ./setup-arch.sh
#   Non-interactive: ZEPHYR_AUTO=1 ./setup-arch.sh
#   Via curl:        curl -fsSL https://raw.githubusercontent.com/.../setup-arch.sh | bash
#
...
#   ZEPHYR_SKIP_FIREWALL=1     - Skip firewall configuration
...
if [[ ! -f /etc/arch-release ]]; then
    log_error "This script is designed for Arch Linux. Use setup-ubuntu.sh for Ubuntu."
    exit 1
fi
...
print_banner() {
    ...
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║                                                           ║"
    echo "║        ZEPHYR CI - Arch Linux Setup (Idempotent)          ║"
    echo "║                                                           ║"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
# Install system packages (idempotent via --needed)
install_packages() {
    ...
    # Install packages (--needed makes it idempotent)
    sudo pacman -S --needed --noconfirm "${packages[@]}"

    log_success "System packages installed"
}

# Setup Docker (idempotent)
setup_docker() {
...
setup_kvm() {
    ...
    if grep -q 'Intel' /proc/cpuinfo; then
        ...
        if [[ ! -f /etc/modules-load.d/kvm.conf ]] || ! grep -q '^kvm_intel$' /etc/modules-load.d/kvm.conf 2>/dev/null; then
            echo "kvm_intel" | sudo tee /etc/modules-load.d/kvm.conf > /dev/null
        fi
    elif grep -q 'AMD' /proc/cpuinfo; then
        sudo modprobe kvm_amd 2>/dev/null || true
        # Make persistent (idempotent check)
        if [[ ! -f /etc/modules-load.d/kvm.conf ]] || ! grep -q '^kvm_amd$' /etc/modules-load.d/kvm.conf 2>/dev/null; then
            echo "kvm_amd" | sudo tee /etc/modules-load.d/kvm.conf > /dev/null
        fi
    fi

    # Add user to kvm group (idempotent)
...
setup_nginx() {
    ...
    # Create sites-available/enabled directories if they don't exist (Arch doesn't have these by default)
